...
class Bunyad_Posts
{
	...
	public function paginate($options = array(), $query = null)
	{
		...
		$args = array(
			...
			'next_text' => '<span class="visuallyhidden">' . _x('Next', 'pagination', 'contentberg') . '</span><i class="fa fa-angle-right"></i>',
			'prev_text' => '<i class="fa fa-angle-left"></i><span class="visuallyhidden">' . _x('Previous', 'pagination', 'contentberg') . '</span>'
		);
	...
	public function excerpt_read_more()
	{
		...
		if (Bunyad::options()->read_more) {
			...
			$text = $this->more_text;
			if (!$text) {
				$text = esc_html__('Read More', 'contentberg');
			}

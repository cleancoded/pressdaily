        ...
        if (!$using_fallback) {
            ...
            if ($featured_image !== null && !is_wp_error($featured_image)) {
                ...
                if ($remove || WPRSS_FTP_Utils::multiboolean($options['remove_ft_image']) === true) {
                    ...
                    $ft_image_url = wp_get_attachment_url($featured_image);
                    // Replace the old featured image URL with the URL of the downloaded featured image
                    $new_content = str_replace($featured_image_source, $ft_image_url, $new_content);
                }

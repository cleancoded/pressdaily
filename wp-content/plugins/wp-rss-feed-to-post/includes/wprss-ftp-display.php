...
final class WPRSS_FTP_Display {
    ...
    public static function post_content( $content ) {
        ...
        global $wp_query;
        if (!empty($wp_query->queried_object) && $wp_query->queried_object->post_type === 'tdb_templates') {
            return $content;
        }
        ...
        if (!empty($source)) {
            ...
            if ( $prepend !== '' && WPRSS_FTP_Display::show_appended_text('prepend', $source, $options) ) {
                $content = WPRSS_FTP_Appender::handle_appended_data( $post, $prepend ) . $content;
            }
            // Add any append text
            if ( $append !== '' && WPRSS_FTP_Display::show_appended_text('append', $source, $options) ) {
                $content .= WPRSS_FTP_Appender::handle_appended_data( $post, $append );
            }


            // If the post's feed source has source_link enabled ...
            if ( WPRSS_FTP_Utils::multiboolean($options['source_link']) &&
                 WPRSS_FTP_Display::show_appended_text('source_link', $source, $options)
            ) {
                // Prepare to data
                ...
                if ( $search !== FALSE ) {
                    ...
                    $linked_text = $text . ' <a href="' . esc_attr( $link ) . '" target="_blank">' . $link . '</a>';
                }

                // Add the generated text to the content
                if ( $linked_text !== '' ) {
                    $position = $options['source_link_position'];
                    $linkMethod = apply_filters('wprss_ftp_add_source_link_method', $position);

                    if( $linkMethod === 'before' ) {
                        $content = '<p>' . $linked_text . '</p>' . ltrim($content);
                    }
                    elseif( $linkMethod === 'after' ) {
                        $content = rtrim($content) . '<p>' . $linked_text . '</p>';
                    }
     ...
     * @since 3.3.2
     */
    public static function show_appended_text($type, $source, array $options) {
        // Check if we can show custom content in the feed
        if (is_feed()) {
            return WPRSS_FTP_Utils::multiboolean( $options['add_content_in_feed'] );
        }

        $singular_only = ($type === 'append' || $type === 'prepend')
            ? WPRSS_FTP_Utils::multiboolean( WPRSS_FTP_Meta::get_instance()->get_meta( $source, 'singular_' . $type ) )
            : WPRSS_FTP_Utils::multiboolean( $options['source_link_singular'] );
        ...
        return !$singular_only || is_singular();
    }

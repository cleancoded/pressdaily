...
add_filter('wprss_ftp_converter_post_content', 'wprss_youtube_iframes', 9);
/**
 * Converts Facebook intermediary links to direct links.
 *
 * @since 3.13.3
 ...
 * @return string
 */
function wprss_youtube_iframes($content)
{
    $newContent = preg_replace_callback('/<iframe.*?src=[\'"](.*?)[\'"].*?>.*?<\\/iframe>/i', function ($m) {
        if (count($m) < 2) {
            return $m[0];
        }

        $testEmbedUrl = function($subject) {
            preg_match('/^http[s]?:\/\/(?:www\.)?youtube\.com\/embed\/(.+)$/i', $subject, $m);

            return count($m) > 0 ? $m[1] : null;
        };

        $testVideoUrl = function($subject) {
            preg_match('/^http[s]?:\/\/(?:www\.)?youtube\.com\/watch(\/?\?v=|\/)(.+)$/i', $subject, $m);

            return count($m) > 1 ? $m[2] : null;
        };

        // Get the video ID - check embed-style URLs first then regular video URLs second
        $videoId = $testEmbedUrl($m[1]);
        if ($videoId === null) {
            $videoId = $testVideoUrl($m[1]);
        }
        // If no video ID found, return the original match
        if ($videoId === null) {
            return $m[0];
        }

        // Replace the '?' URL query initiator character with a '&' continuation character
        $videoId = htmlspecialchars_decode($videoId);
        $videoId = str_replace('?', '&', $videoId);

        // Return an embed shortcode
        return "\n[embed]https://youtube.com/watch?v={$videoId}[/embed]\n";
    }, $content);

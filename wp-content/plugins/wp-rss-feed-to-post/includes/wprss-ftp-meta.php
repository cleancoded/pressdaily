...
/*===== CONSTRUCTOR AND SINGLETON GETTER ==================================================================*/
	...
	public function disable_visual_editor($use_visual_editor) {
        ...
        if ( $screen->base == 'post' && $screen->post_type == 'wprss_feed') {
            $action = !empty($_GET['action']) ? $_GET['action'] : null;

            // If editing, disable visual editors
            if ($action === 'edit') {
                // Check the visual editors option for the feed source
                $disable_visual_editor = $this->get_meta(get_the_ID(), 'disable_visual_editor');
...
#== META BOX RENDERERS =================================================================================
	...
	public function render_custom_fields( $post, $field_id, $field, $meta ) {
		...
		$namespaces_array = array_merge(
			...
			array_combine(
                array_map('strtolower', $saved_namespaces['names']),
                $saved_namespaces['names']
            )
		);
		...
		for ( $i = 0; $i < count( $rss_namespaces ); $i++ ) {
			...
			$namespace = strtolower($rss_namespaces[$i]);
			$tag = $rss_tags[$i];
	...
	public function save_post_meta( $post_id, $post ) {
		...
		if ( $post_id && ! empty( $_POST[ self::META_PREFIX.'post_taxonomy' ] ) ) {
			...
			for ( $i = 0; $i <= $n; $i++ ) {
				...
				$tax['auto'] = isset($autos[$i]) ? $autos[$i] : '';
...
#== MISC ======================================================================================
	...
	public static function multisite_fix() {
        ...
        foreach ($site_ids as $site_id) {
            ...
            foreach ($feed_sources as $post) {
                $post_site = get_post_meta($post->ID, WPRSS_FTP_Meta::META_PREFIX . 'post_site', true);

                if (empty($post_site) || strtolower(strval($post_site)) === 'false') {
                    update_post_meta($post->ID, WPRSS_FTP_Meta::META_PREFIX . 'post_site', $site_id);
                }
            }
        }
